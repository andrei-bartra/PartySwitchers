...
def raw_data_cleaning(tables, fields, table_list):
    ...
    pos_con = fields.campo.str.contains(r'^prev_pos|^post_pos')
        ...
        for f in t_fields:
            ...
            if f in year_fields:
                tables[t][f] = pd.to_numeric(tables[t][f]. \
                                  fillna(' '). \
                                  replace(' ','0'). \
                                  replace('HASTA LA ACTUALIDAD', '2021')). \
    ...
    # Pevious positions
    pos_fields = [field for field in pos_fields if 'prev_' in field]
    t = 'position_record'
...
def network_table(tables):
    ...
    # Filtering double party members
    ntwk = ntwk.drop_duplicates(subset=['id_hdv', 'year', 'source', 'target'])
